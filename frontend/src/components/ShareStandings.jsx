...
const TITLE = "Classificação · Campeonato Brejolandense";
...
async function deliver(blob, filename, type) {
...
// Botão que abre uma janela com a prévia da tabela e opções de compartilhar
// como imagem, PDF ou texto.
export default function ShareStandingsButton({ data, className, children }) {
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Compartilhar classificação">
        {error ? (
          ...
          <div className="space-y-4">
            <img src={previewUrl} alt="Prévia da classificação" className="w-full rounded-xl border border-white/10" />
            <div className="grid grid-cols-2 gap-3">
              <button
                type="button"
                onClick={() => deliver(files.png, "classificacao-brejolandense.png", "image/png")}
              ...
              <button
                type="button"
                onClick={() => deliver(files.pdf, "classificacao-brejolandense.pdf", "application/pdf")}
                className="btn-primary text-sm"
              >
                {sharePdf ? "📄 PDF" : "⬇️ Baixar PDF"}
              </button>
            </div>
            <button type="button" onClick={() => shareStandings(data)} className="btn-ghost text-sm w-full">
